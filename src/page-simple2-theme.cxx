...
void simple2_theme_t::render_notebook(cairo_t * cr, theme_notebook_t const * n) const {
	...
	CHECK_CAIRO(cairo_save(cr)); // save #0
	...
	{
		CHECK_CAIRO(cairo_save(cr)); // save #3
	...
	if (n->has_selected_client) {
		if (n->selected_client.is_iconic) {
			...
			{
				CHECK_CAIRO(cairo_save(cr)); // store #1
				cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
				if (backgroun_px != nullptr) {
					CHECK_CAIRO(cairo_set_source_surface(cr, backgroun_px, -n->root_x, -n->root_y));
				} else {
				...
				CHECK_CAIRO(cairo_restore(cr)); // restore #1
			}
...

	{
		...
		CHECK_CAIRO(cairo_save(cr)); // save #2
		...
		CHECK_CAIRO(cairo_restore(cr)); // restore #2
	...
	CHECK_CAIRO(cairo_restore(cr)); // restore #0
...
void simple2_theme_t::render_notebook_selected(
...

	{
		...
		CHECK_CAIRO(::cairo_set_source_rgb(cr, data.tab_color.r*border_factor, data.tab_color.g*border_factor, data.tab_color.b*border_factor));
		CHECK_CAIRO(cairo_stroke(cr));
...
//	}
	...
	/** draw title **/
	{
		...
		CHECK_CAIRO(cairo_save(cr)); // save #1
...
void simple2_theme_t::update(int width, int height) {
	create_background_img(width, height);
}
...
void simple2_theme_t::create_background_img(int width, int height) {
	...
	if (has_background) {
		...
		cairo_surface_t * image_background_s = cairo_image_surface_create(CAIRO_FORMAT_RGB24, width, height);
		...
		cairo_t * cr = cairo_create(backgroun_px);
		cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
...
void simple2_theme_t::render_popup_split(cairo_t * cr, theme_split_t const * s,
	...
	cairo_rectangle_arc_corner(cr, rect0.x, rect0.y, rect0.w, rect0.h, 10.0, CAIRO_CORNER_ALL);
	CHECK_CAIRO(cairo_stroke(cr));

	CHECK_CAIRO(::cairo_set_source_rgba(cr, 1.0, 1.0, 1.0, 1.0));
	cairo_rectangle_arc_corner(cr, rect1.x, rect1.y, rect1.w, rect1.h, 10.0, CAIRO_CORNER_ALL);
	CHECK_CAIRO(cairo_stroke(cr));
...
auto simple2_theme_t::workspace_switch_popup(string const & workspace_name) const -> cairo_surface_t *
{
	cairo_surface_t * popup = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, 600, 100);

	cairo_t * cr = cairo_create(popup);
	cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
	cairo_set_source_rgba(cr, 0.0, 0.0, 0.0, 0.0);
	cairo_paint(cr);
...
auto simple2_theme_t::get_background() const -> cairo_surface_t * {
	return backgroun_px;
}
